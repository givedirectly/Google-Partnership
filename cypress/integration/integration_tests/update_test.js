describe('Integration test', () => {
  it('Submits multiple new values at once', () => {
    ...
    cy.get('[id="current poverty weight"]').should('have.text', 'current poverty weight: 0.2');
    cy.get('[id="current damage threshold"]').should('have.text', 'current damage threshold: 0.5');
  });
  ...
  it.only('Submits a new weight', () => {
    ...
    cy.get('[id="current poverty weight"]').should('have.text', 'current poverty weight: 0.2');
    cy.get('[id="current damage weight"]').should('have.text', 'current damage weight: 0.8');
  });
  ...
  it('Submits new weights that do not add to 1', () => {
    ...
    cy.get('[id="error"]').should('have.text', 'ERROR: poverty weight and damage weight must add up to 1.0' );
  });

  it('Submits a new threshold value', () => {
    cy.visit(host);

    cy.get('[id="current poverty threshold"]').should('have.text', 'current poverty threshold: 0.3');

    cy.get('[id="poverty threshold"]').type('0.5');
    cy.get('[id="update"]').click();

    cy.get('[id="current poverty threshold"]').should('have.text', 'current poverty threshold: 0.5');
  ...
  it('Submits an invalid threshold value (empty)', () => {
    ...
    cy.get('[id="error"]')
        .should('have.text', 'ERROR: poverty threshold must be between 0.00 and 1.00');
  });
