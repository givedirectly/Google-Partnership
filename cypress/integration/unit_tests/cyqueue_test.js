...
describe('Tests of Cypress ordering, both for self-documentation and detection of behavior changes', () => {
  it('Tests that assumptions of ordering are correct', () => {
    const threadOrdering = [];
    let resolveFunction = null;
    new Promise((resolve) => resolveFunction = resolve).then(() => threadOrdering.push('promise'));
    ...
    cy.wait(0).then(() => threadOrdering.push('wait'))
        .then(() => expect(threadOrdering).to.eql(['resolved', 'get', 'promise', 'wait']));
  });
  ...
  it('Tests that promise can finish first if dom element not found', () => {
    ...
    cyQueue(resolveFunction).then(() => threadOrdering.push('resolved'));
    cy.get('#new-id').then(() => threadOrdering.push('get'))
        .then(() => expect(threadOrdering).to.eql(['resolved', 'promise', 'get']));
  });
});
