...
export default defineConfig({
  ...
  setupNodeEvents(on, config) {
      ...
      /**
       * Defines "tasks" that can be run using cy.task(). The name of each task
       * is the function name. These tasks are invoked in
       * cypress/support/index.js in a "before" hook, but they can theoretically
       * be called anywhere that cy.task() is legal to invoke.
       */
      on('task', {
        /**
         * The following two functions use service account credentials (stored
         * in the json file pointed to by the environment variable
         * GOOGLE_APPLICATION_CREDENTIALS) to generate tokens that can be used
         * by production code to authenticate with Firebase/EarthEngine. These
         * tokens are different from the actual service account credentials,
         * which cannot be used by a client-side Javascript application.
         *
         * We do these initializations in this plugin because creating such a
         * custom token that's easy to pass around can best be done in libraries
         * that are only available on Node, which runs in a "server"-like
         * environment, not client-side Javascript (for Firebase, the Firebase
         * Admin SDK is only available in Node). Even Cypress tests, though they
         * appear to run in Node, are actually browserified, and the above
         * modules don't work there. Thus, we use genuine Node modules, and then
         * pass the created tokens back out to the test, where it can use them
         * (in the case of Firebase) and also set values for the production code
         * to use.
         */

        /**
         * Produces a Firebase token that can be used for our Firestore
        database.
         * Because the database belongs to a test-only user, this user is given
        free
         * reign to do whatever it likes. Also deletes all test data older than
        7
         * days, so there isn't indefinite build-up if tests are frequently
        aborted
        ...
        getEarthEngineToken() {
          return generateEarthEngineToken(false).then(
              (data) => data.accessToken);
        },

        /**
         * Writes disasters data (retrieved using
         * {@link retrieveFirestoreDataForTest} into the current test root,
         * which lives under the root collection {@link testPrefix}.
         *
        ...
        populateTestFirestoreData(currentTestRoot) {
          // We use a test app, created using normal firebase, versus a test
          // admin app, because objects like GeoPoint are not compatible across
          // the libraries. By using the same library, we're able to copy the
          // data from prod to test with no modifications.
        ...
        /**
         * Deletes all test data under the test/currentTestRoot tree. For use
         * after a test case is completed.
         * @param {string} currentTestRoot document name directly underneath
         *     test/
         * @return {Promise<null>} Promise that resolves when deletion is
         *     complete
         */
         ...
         * CI console versus in the browser. Only needed for Chrome browser,
         * since ELECTRON_ENABLE_LOGGING makes Electron display everything.
         * @param {string} str
         * @return {null}
         */
        logg(str) {
